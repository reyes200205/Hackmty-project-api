...
def compute_mfcc(x: np.ndarray, sample_rate: int, n_mfcc: int = N_MFCC, n_mels: int = N_MELS) -> np.ndarray:
...
def _autocorr_pitch(frames: np.ndarray, sample_rate: int) -> tuple[np.ndarray, np.ndarray]:
    """Pitch (F0) y fuerza de voicing por frame via autocorrelacion (Wiener-Khinchin, vectorizado)."""
    ...
    norm_peak = peak_val / (energy + 1e-12)
...
def extract_features(x: np.ndarray, sample_rate: int) -> dict[str, float]:
...
def feature_vector(x: np.ndarray, sample_rate: int) -> np.ndarray:
